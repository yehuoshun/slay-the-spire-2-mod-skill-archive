...
{
    private static void Postfix(ref List<EncounterModel> __result)
    {
        foreach (var type in Assembly.GetExecutingAssembly().GetTypes())
        {
            if (type.GetCustomAttribute<RegisteredEncounterAttribute>() != null)
                __result.Add((EncounterModel)Activator.CreateInstance(type)!);
        }
    }
}
```
